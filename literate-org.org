...
#+BEGIN_SRC elisp
(defun literate-org-point-at-code-block-of-tangled-location (org-buffer file-name point)
  (when (file-exists-p file-name)
    (with-current-buffer (find-file-noselect file-name)
      (save-excursion
        (goto-char point)
        (let* ((column-index (- (point) (line-beginning-position)))
          ...
          (when (null org-point)
            (return-from literate-org-point-at-code-block-of-tangled-location nil))
