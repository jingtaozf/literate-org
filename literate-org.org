...
(defun literate-org-tangle-buffer ()
  "Tangle every code block in the current buffer, then Black-format the
.py output directories.

Per-block Black via `python-black-org-mode-block' (in `before-save-hook')
is necessary but not sufficient: Black's inter-definition rules — most
visibly, two blank lines between top-level defs — require seeing the
*concatenated* tangle output, not individual blocks. Running Black on
the output directories closes that gap so the tangled .py is
byte-identical to a manually `make lint'-ed version.
...
spuriously skip tangle in that window."
  (when (and (buffer-file-name)
    ...
    (let* ((inhibit-message t)
      ...
      (dolist (d py-dirs)
        (ignore-errors
          (call-process "poetry" nil nil nil "run" "black" "--quiet" d))))))
#+END_SRC
