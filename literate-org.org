...
Definition / class / decorated definition pull the =:name= field;
assignments and calls render the source text (truncated by
=literate-org-max-title-size= to a left-hand-side or callee fall-back);
the trivial node types (=try=, =while=, =string=, =comment=, =import=,
=import_from_statement=) just return a constant label;
=expression_statement= recurses into its first child.

Same shape as the Rust / TSX sections: two name-bearing nodes plus
six trivial constants fold into two =defconst= alists consulted by
one catch-all =cl-defmethod=. The five nodes with per-shape logic
(=decorated_definition= chains decorators; =assignment= /
=augmented_assignment= / =call= truncate via
=literate-org-max-title-size=; =expression_statement= unwraps) keep
their own specialised methods.
...
blocks fall through to TSX via =literate-org-alias-language= — note
that even though this subsection lives under =**** vue=, every
=cl-defmethod= dispatches on =(eql tsx)=. Same shape as the Python
table above: name-bearing nodes (function / type alias / enum /
interface / variable_declarator) pull the =:name= field; constant
labels for trivial nodes; =export_statement= and =expression_statement=
recurse one level into a wrapped node; =lexical_declaration= prepends
the =:kind= keyword.

Same shape as the Rust section: five name-bearing nodes plus three
constants are folded into two =defconst= alists, one catch-all
=cl-defmethod= consults them; the three nodes that carry per-shape
logic (=export_statement=, =lexical_declaration=,
=expression_statement=) keep their own specialised methods.
