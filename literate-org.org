...
Dispatch table for =literate-org-node-name=. Name-bearing items pull
=:name=; =impl_item= renders =impl <Trait> for <Type>= when =:trait=
is present, else =impl <Type>=; =macro_invocation= prefers =:macro=
and falls back to source text; =expression_statement= recurses into
its first child. Constant labels cover use / extern / comment /
attribute nodes that lack useful names.
