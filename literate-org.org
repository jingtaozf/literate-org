...
#+BEGIN_SRC elisp
(defcustom literate-org-ignored-dirs '("__pycache__" ".git" ".vscode" ".idea" ".venv" "__pycache__" "__marimo__" "node_modules")
...
#+BEGIN_SRC elisp
;; function_definition — the function's :name field
...
;; class_definition — the class's :name field
(cl-defmethod literate-org-node-name ((language (eql python))
...
;; assignment — the source text, or its :left child when too long
(cl-defmethod literate-org-node-name ((language (eql python))
...
;; augmented_assignment — same shape as assignment
(cl-defmethod literate-org-node-name ((language (eql python))
...
;; call — the source text, or its :function child when too long
(cl-defmethod literate-org-node-name ((language (eql python))
...
;; Constant labels for control-flow / scalar nodes that have no useful name.
(cl-defmethod literate-org-node-name ((language (eql python))
...
Rust top-level items have a clean tree-sitter grammar — every item type
(=function_item=, =struct_item=, =enum_item=, =trait_item=, =union_item=,
=mod_item=, =const_item=, =static_item=, =type_item=, =macro_definition=,
=impl_item=, =foreign_mod_item=) is a direct child of the =source_file=
root, and most carry a =:name= field. The dispatch table mirrors the
Python one above; only a handful of Rust-specific shapes need
hand-coding:

- Outer attributes (=#[derive(...)]=) and doc comments (=///=) precede
  the item they decorate, so =next-code-block-index= slides past them
  and lets the item's heading absorb them.
- Multiple =use= / =extern crate= declarations get folded into a single
  "Use declarations" block, matching the way Python collapses
  =import_statement= runs.
- Inner attributes (=#![...]=) at the very top of the file have to be
  skipped before scanning for the =//!= module doc, otherwise the doc
  is lost when a crate opens with =#![allow(dead_code)]=.
- Top-level macro invocations come in two shapes: =lazy_static! { ... }=
  parses as a direct =macro_invocation=, while =include_str!(...);=
  parses as an =expression_statement= wrapping a =macro_invocation=.
  Both are emitted as "Macro call <name>" blocks via the =:macro= field.

***** file extension and module comment

The extension is =.rs=. The module comment is the contiguous run of
=//!= lines at the top of the file, *after* any leading
=inner_attribute_item= / =attribute_item= nodes — this is the gap that
bit the prototype on files starting with =#![allow(...)]=.
...
The Rust dispatch table for =literate-org-node-name=. Name-bearing
items pull =:name=; =impl_item= renders =impl <Trait> for <Type>= when
=:trait= is present and =impl <Type>= otherwise; =macro_invocation=
prefers =:macro= and falls back to the source text;
=expression_statement= recurses into its first child (same unwrap rule
as Python). Constant labels cover use / extern / comment / attribute
nodes that don't carry useful names.

#+BEGIN_SRC elisp
;; Items that expose a :name field — flat dispatch.
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql function_item)) node)
  (format "Function %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql struct_item)) node)
  (format "Struct %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql enum_item)) node)
  (format "Enum %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql trait_item)) node)
  (format "Trait %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql union_item)) node)
  (format "Union %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql const_item)) node)
  (format "Const %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql static_item)) node)
  (format "Static %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql type_item)) node)
  (format "Type %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql mod_item)) node)
  (format "Mod %s" (tsc-node-text (tsc-get-child-by-field node :name))))
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql macro_definition)) node)
  (format "Macro %s" (tsc-node-text (tsc-get-child-by-field node :name))))

;; impl_item — :type required, :trait optional.
(cl-defmethod literate-org-node-name ((_language (eql rust))
...
;; "include_str", "my_crate::register").
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql macro_invocation)) node)
  (let ((macro-node (ignore-errors (tsc-get-child-by-field node :macro))))
    (if macro-node
      (format "Macro call %s" (tsc-node-text macro-node))
      (format "Macro call %s" (tsc-node-text node)))))

;; expression_statement — recurse into the wrapped expression. Same unwrap
;; rule as Python's expression_statement handler.
...
;; Constant labels for trivial nodes.
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql use_declaration)) _node)
  "Use")
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql extern_crate_declaration)) _node)
  "Extern crate")
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql foreign_mod_item)) _node)
  "Extern block")
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql attribute_item)) _node)
  "Attribute")
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql inner_attribute_item)) _node)
  "Inner attribute")
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql line_comment)) _node)
  "Comment")
(cl-defmethod literate-org-node-name ((_language (eql rust))
                                      (_node-type (eql block_comment)) _node)
  "Comment")
#+END_SRC

***** get the next code block index

Rust's =next-code-block-index= keeps three sets of node types: real
*items* that become their own block, *use-like* declarations that get
folded into one "Use declarations" block, and *preludes* (attributes,
comments) that slide forward and merge with the next item. An
=expression_statement= whose first child is a =macro_invocation= also
counts as an item — that's how =include_str!(...);= and friends get
their own block instead of disappearing into the fall-through.
...
#+BEGIN_SRC elisp
;; function_declaration — the function's :name field
(cl-defmethod literate-org-node-name ((language (eql tsx))
                                      (node-type (eql function_declaration)) node)
  (format "Function %s" (tsc-node-text (tsc-get-child-by-field node :name))))

;; Constant labels for nodes with no useful name field.
(cl-defmethod literate-org-node-name ((language (eql tsx))
                                      (node-type (eql string)) node)
  "String")
...
;; type_alias_declaration / enum_declaration / interface_declaration — :name field
(cl-defmethod literate-org-node-name ((language (eql tsx))
