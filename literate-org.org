...
#+BEGIN_SRC elisp
(defun update-org-code-block (update-func language)
  (save-mark-and-excursion
    (pcase (org-src--contents-area (org-element-at-point))
      (`(,beg ,end ,_)
        (let ((buf (current-buffer))
              (end (- end 1))
              (temp-file (format "/tmp/%s.%s" (make-symbol "literate-org")
...
#+BEGIN_SRC elisp
(cl-defun literate-org-parse-tree-for-current-file
    (&key (language (treesit-parser-language (car (treesit-parser-list)))))
  (let ((language (tree-sitter-require language))
        (parser (tsc-make-parser)))
    (tsc-set-language parser language)
...
(defcustom literate-org-max-title-size 80
  ...
  :group 'literate-org)
#+END_SRC
**** iterate a directory to import source files
#+BEGIN_SRC elisp
(cl-defun literate-org-import (&key (level (or (org-current-level) 0))
  ...
  (let* ((module-name (or module-name
         ...
         (language (or (aand (org-entry-get (point) "LITERATE_ORG_LANGUAGE" t)
                             (intern it))
                       (intern (read-from-minibuffer "Which language: "))))
         (path-extension (literate-org-file-extension language))
         (path-ext-regx (concat "\." path-extension "$"))
         (module-path (or module-path
                          (let* ((root-path (org-entry-get
                                             (point) "LITERATE_ORG_ROOT_MODULE_PATH" t))
                                 (path (when root-path
                                         (concat root-path "/"
                            ...
                            (when path
                              (if (f-directory? path)
                                path
                                (let ((path (concat path "." path-extension)))
                                  (when (f-exists-p path)
                                    path)))))
                          (org-entry-get (point) "LITERATE_ORG_MODULE_PATH" t)))
         (new-level (1+ level)))
    (if (f-directory-p module-path)
      (progn
        ;; TODO: cleanup empty directories.
        (dolist (file (directory-files module-path t path-ext-regx))
      ...
      (literate-org-import-source-file-to-org
       language module-path level module-name :with-org-section nil))))
#+END_SRC
...
#+BEGIN_SRC elisp
(cl-defun literate-org-get-code-blocks-in-file
    (file &key (language (treesit-parser-language (car (treesit-parser-list)))))
  (with-current-buffer (find-file-noselect file)
    (let* ((tree (literate-org-parse-tree-for-current-file :language language))
           (root (tsc-root-node tree))
  ...
  (let* ((new-level (1+ level))
    ...
    (when with-org-section
      ...
      (let* ((module-root-path (org-entry-get (point) "LITERATE_ORG_ROOT_MODULE_PATH" t))
             ...
             (module-file-name (dired-replace-in-string
                                "\\." "/"
                                (if (> (length root-module) 0)
