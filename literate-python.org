...
*** json
:PROPERTIES:
:LITERATE_PYTHON_MODULE: literate_python.json
:header-args: :tangle ./literate_python/json.py
:END:
**** Import statements
#+BEGIN_SRC python
...
def _pyinspect_trim(obj, elem_cap, str_cap):
    ...
    if type(obj) in (dict, tuple, list):
        jsondump = json.dumps(trim_seq(obj, elem_cap), indent=4)
        if len(obj) > elem_cap:
            return f"{jsondump[:-1]}    ...\n{jsondump[-1]}"
        return jsondump
...
def _pyinspect_json(obj):
    return json.dumps(
        _pyinspect(obj), indent=4, default=lambda o: _pyinspect(o)["value"]
    )
