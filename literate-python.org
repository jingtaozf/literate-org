...
#+PROPERTY: LITERATE_PYTHON_SERVER http://localhost:7332/
...
- [[#the-emacs-library][The Emacs library]]
  ...
  - [[#source-code-execution][Source Code Execution]]
    ...
    - [[#execute-code-in-remote-server][execute code in remote server]]
...
#+END_SRC
**** Initialize in-memory module storage
#+BEGIN_SRC python
...
from loader import register_literate_module_finder
...
(cl-defun literate-python-request (path &key params data (type "GET"))
  ...
  (let* ((request-backend 'url-retrieve)
         (server (or (save-current-buffer
                       (when-let ((base-buffer (buffer-base-buffer (current-buffer))))
                        ;; have to switch to org buffer to access property value.
                        (set-buffer base-buffer))
                       (org-entry-get (point) "LITERATE_PYTHON_SERVER" t))
                     literate-python-rest-server))
         resp)
    (request (concat server path)
        :params params
        :parser 'json-read
        :type type
